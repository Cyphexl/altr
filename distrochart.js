...
function makeDistroChart(settings) {
  ...
  chart.settings = {
    ...
    margin: { top: 33, right: 0, bottom: 66, left: 80 },
  ...
  chart.update = function() {
    ...
    chart.objs.g
      .select(".y.axis")
      .call(chart.objs.yAxis.innerTickSize(-chart.width));
        ...
        if (cPlot.objs.points) {
          if (dOpts.plotType == "beeswarm") {
            ...
            var maxWidth = Math.floor(
              chart.xScale.rangeBand() / dOpts.pointSize
            );
            var ptsObj = {};
            var cYBucket = null;
            //  Bucket points
    ...
    chart.dataPlots.preparePlots = function() {
      ...
      if (dOpts.showLines && dOpts.showLines.length > 0) {
        ...
        for (var line in dOpts.showLines) {
          ...
          chart.dataPlots.objs.lines[cMetric].line = d3.svg
            .line()
      ...
      chart.dataPlots.objs.g = chart.objs.g
        .append("g")
        .attr("class", "metrics-bars");
      if (dOpts.showBars && dOpts.showBars.length > 0) {
        chart.dataPlots.objs.bars = {};
        var cMetric;
        for (var bar in dOpts.showBars) {
          cMetric = dOpts.showBars[bar];
          chart.dataPlots.objs.bars[cMetric] = {};
          chart.dataPlots.objs.bars[cMetric].values = [];
          for (var cGroup in chart.groupObjs) {
            chart.dataPlots.objs.bars[cMetric].values.push({
              x: cGroup,
              y: chart.groupObjs[cGroup].metrics[cMetric]
            });
          }
          chart.dataPlots.objs.bars[cMetric].bar = d3.svg
            .bar()
            .interpolate("cardinal")
            .y(function(d) {
              console.log(d);
          ...
          chart.dataPlots.objs.bars[cMetric].g = chart.dataPlots.objs.g
            .append("path")
            .attr("class", "bar " + cMetric)
            .attr("data-metric", cMetric)
            .style("fill", "red")
            .style("stroke", chart.colorFunct(cMetric));
        }
